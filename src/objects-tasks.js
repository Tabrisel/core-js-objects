...
function mergeObjects(objects) {
  ...
  objects.forEach((obj) => {
    Object.entries(obj).forEach((item) => {
      ...
      const value = item[1];
      if (key in merged) {
        merged[key] += value; // if exist
      } else {
        merged[key] = value; // set new
      }
    });
  });
...
function compareObjects(obj1, obj2) {
  ...
  return keys.every((value) => obj1[value] === obj2[value]);
}
...
function sellTickets(queue) {
  return queue.reduce((acc, cur) => (cur > 25 ? acc - cur : acc + cur), 0) >= 0;
}
...
function Rectangle(width, height) {
  ...
  this.getArea = function calculate() {
...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    ...
    if (a.country > b.county) return 1;
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((item) => {
    ...
    return map.get(key) ? map.get(key).push(value) : map.set(key, [value]);
  });
